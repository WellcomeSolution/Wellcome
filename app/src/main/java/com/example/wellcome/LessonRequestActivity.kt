...
import com.example.wellcome.utils.db.LessonRequest
import com.example.wellcome.utils.db.TripRequest
import com.google.android.material.datepicker.MaterialDatePicker
import kotlinx.android.synthetic.main.activity_lesson_request.*
...
class LessonRequestActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        request_button_lesson.setOnClickListener{
            var request = LessonRequest(startDate.toString(), endDate.toString(),
                editText_message.text.toString(),
                request_phone.text.toString(),
                request_name.text.toString()
            )
            db.requestLessonDao().insert(request)
            finish()
        }
    }
}